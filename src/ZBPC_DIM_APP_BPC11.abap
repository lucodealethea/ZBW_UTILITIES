...
elseif :v_sys = 'TTAB100' or :v_sys = 'TTPB100' THEN
lt_max_pos=
SELECT
max(DD.POSITION) as MAX_POS
FROM :lt_metadata
INNER JOIN DD03L AS DD ON DD.TABNAME = CUSTOM_VIEW AND DD.FIELDNAME = DIMENSION AND DD.AS4LOCAL = 'A'
;
...
META.DIMENSION||' : '||DD2.ROLLNAME||' ;' AS FIELDS_TF
FROM :lt_metadata as META
INNER JOIN DD03L AS DD ON DD.TABNAME = META.CUSTOM_VIEW AND META.DIMENSION = DD.FIELDNAME
LEFT OUTER JOIN DD03L AS DD2 ON DD2.TABNAME = META."MDATA_BW_TABLE" AND META.BW_FIELDNAME = DD2.FIELDNAME AND DD2.AS4LOCAL = 'A'
/*
AND DD.TABCLASS = 'VIEW'
*/
...
endmethod.
endclass. "end class zcl_bpc_metadata definition
